...
CREATE INDEX CONCURRENTLY IF NOT EXISTS "orders_profile_id_status_created_at_idx" ON "orders" ("profile_id", "status", "created_at");

-- Add productId+isActive index on product_variants
CREATE INDEX CONCURRENTLY IF NOT EXISTS "product_variants_product_id_is_active_idx" ON "product_variants" ("product_id", "is_active");

-- Add orderId+isReturned index on order_items
CREATE INDEX CONCURRENTLY IF NOT EXISTS "order_items_order_id_is_returned_idx" ON "order_items" ("order_id", "is_returned");
