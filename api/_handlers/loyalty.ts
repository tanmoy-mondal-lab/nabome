...
async function handlePoints(_req: Request, ctx: RequestContext): Promise<Response> {
  ...
  try {
    ...
    let points = await prisma.loyalty_points.findUnique({
      where: { profileId: ctx.userId },
      include: { transactions: { orderBy: { createdAt: "desc" }, take: 10 } },
    });
    if (!points) {
      points = await prisma.loyalty_points.create({
        data: { profileId: ctx.userId, points: 0, tier: "bronze", lifetimePoints: 0 },
        include: { transactions: true },
      });
    }
    const tiers = await prisma.loyalty_tiers.findMany({ where: { isActive: true }, orderBy: { minPoints: "asc" } });
...
async function handleTransactions(_req: Request, ctx: RequestContext): Promise<Response> {
  ...
  try {
    const prisma = getPrisma(ctx.env);
    const points = await prisma.loyalty_points.findUnique({ where: { profileId: ctx.userId } });
...
async function handleRedeem(req: Request, ctx: RequestContext): Promise<Response> {
  ...
  try {
    ...
    const prisma = getPrisma(ctx.env);
    const lp = await prisma.loyalty_points.findUnique({ where: { profileId: ctx.userId } });
    if (!lp || lp.points < points) return badRequest("Insufficient points");
    await prisma.$transaction(async (tx) => {
      await tx.loyalty_points.update({ where: { id: lp.id }, data: { points: { decrement: points } } });
...
async function handleAdminList(ctx: RequestContext): Promise<Response> {
  try {
    const prisma = getPrisma(ctx.env);
    const all = await prisma.loyalty_points.findMany({
...
async function handleAdminAdjust(req: Request, ctx: RequestContext): Promise<Response> {
  try {
    ...
    const prisma = getPrisma(ctx.env);
    let lp = await prisma.loyalty_points.findUnique({ where: { profileId } });
    if (!lp) {
      lp = await prisma.loyalty_points.create({ data: { profileId, points: 0, tier: "bronze", lifetimePoints: 0 } });
    }
    await prisma.$transaction(async (tx) => {
      await tx.loyalty_points.update({ where: { id: lp!.id }, data: { points: { increment: points } } });
      await tx.loyalty_transactions.create({
