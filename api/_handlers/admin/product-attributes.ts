...
async function handleList(req: Request, env: any): Promise<Response> {
  ...
  try {
    ...
    const items = await prisma.product_attributes.findMany({ where: where as never, orderBy: { name: "asc" } });
...
async function handleCreate(req: Request, env: any): Promise<Response> {
  ...
  try {
    ...
    const product = await prisma.products.findUnique({ where: { id: body.productId }, select: { id: true } });
    if (!product) return notFound("Product not found");
    const item = await prisma.product_attributes.create({
      data: {
...
async function handleUpdate(id: string, req: Request, env: any): Promise<Response> {
  try {
    const prisma = getPrisma(env);
    const body = await req.json();
    const existing = await prisma.product_attributes.findUnique({ where: { id } });
    ...
    const updated = await prisma.product_attributes.update({ where: { id }, data: data as never });
...
async function handleDelete(id: string, env: any): Promise<Response> {
  try {
    ...
    if (!existing) return notFound("Attribute not found");
    await prisma.product_attributes.delete({ where: { id } });
