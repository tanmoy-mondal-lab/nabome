...
import { Package, Search, CheckCircle, XCircle, RefreshCw, Trash2, AlertCircle, Edit2 } from "lucide-react";
import { useToast } from "../../components/Toast";
import { generateMockAdminProducts } from "../../lib/mockAdminData";
...
const statusConfig: Record<string, { label: string; color: string; bg: string }> = {
  ...
  soft_deleted: { label: "Deleted", color: "#e74c3c", bg: "#e74c3c18" },
};
...
export default function AdminProducts() {
  ...
  useEffect(() => { setProducts(generateMockAdminProducts()); }, []);

  const handleApprove = (id: string) => {
    setProducts((prev) => prev.map((p) => p.id === id ? { ...p, status: "published" as AdminProduct["status"] } : p));
    showToast("Product approved and published!");
  };

  const handleReject = (id: string) => {
    const reason = prompt("Rejection reason:");
    setProducts((prev) => prev.map((p) => p.id === id ? { ...p, status: "rejected" as AdminProduct["status"], rejectionReason: reason || "Quality standards not met" } : p));
    showToast("Product rejected.");
  };

  const handleRestore = (id: string) => {
    setProducts((prev) => prev.map((p) => p.id === id ? { ...p, status: "draft" as AdminProduct["status"] } : p));
    showToast("Product restored from trash!");
  };
  ...
  const filters = ["all", "published", "pending_approval", "draft", "rejected", "soft_deleted"];
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
        {filters.map((f) => <button key={f} onClick={() => setFilter(f)} style={chipS(filter === f)}>{f === "pending_approval" ? "Pending" : f === "soft_deleted" ? "Deleted" : f.charAt(0).toUpperCase() + f.slice(1)}</button>)}
      </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", zIndex: 1000, display: "grid", placeItems: "center", padding: 20 }}>
          <div className="glass" style={{ padding: 32, borderRadius: "var(--radius-xl)", maxWidth: 420, width: "100%", textAlign: "center" }}>
            ...
            <div style={{ display: "flex", gap: 12, justifyContent: "center" }}>
              <button onClick={() => handlePermanentDelete(confirmation)} className="premium-button" style={{ background: "#e74c3c", borderColor: "#e74c3c", padding: "0 24px", fontSize: ".85rem" }}>Delete Permanently</button>
            ...
            {filtered.map((p, i) => {
              ...
              return (
                <motion.tr key={p.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.02 }}
                  ...
                  <td style={{ padding: "12px 14px", fontSize: ".82rem", color: "var(--muted)" }}>{p.vendorName}</td>
                  ...
                  <td style={{ padding: "12px 14px", textAlign: "center" }}>
                    ...
                    {p.status === "rejected" && p.rejectionReason && (
                      <p style={{ color: "var(--muted)", fontSize: ".68rem", marginTop: 2 }} title={p.rejectionReason}>Reason set</p>
                    )}
                  </td>
                  <td style={{ padding: "12px 14px", textAlign: "right" }}>
                    <div style={{ display: "flex", gap: 4, justifyContent: "flex-end" }}>
                      {p.status === "pending_approval" && (
                        <>
                          <button onClick={() => handleApprove(p.id)} title="Approve" style={{ width: 30, height: 30, border: "none", background: "#2ecc71", color: "#fff", cursor: "pointer", borderRadius: "var(--radius)", display: "grid", placeItems: "center" }}><CheckCircle size={12} /></button>
                          <button onClick={() => handleReject(p.id)} title="Reject" style={{ width: 30, height: 30, border: "none", background: "#e74c3c", color: "#fff", cursor: "pointer", borderRadius: "var(--radius)", display: "grid", placeItems: "center" }}><XCircle size={12} /></button>
                        </>
                      )}
                      {p.status === "soft_deleted" && (
                        <button onClick={() => handleRestore(p.id)} title="Restore" style={{ width: 30, height: 30, border: "1px solid var(--line)", background: "transparent", color: "#2ecc71", cursor: "pointer", borderRadius: "var(--radius)", display: "grid", placeItems: "center" }}><RefreshCw size={12} /></button>
                      )}
                      {(p.status === "soft_deleted" || p.status === "rejected") && (
                        <button onClick={() => setConfirmation(p.id)} title="Delete Permanently" style={{ width: 30, height: 30, border: "1px solid var(--line)", background: "transparent", color: "#e74c3c", cursor: "pointer", borderRadius: "var(--radius)", display: "grid", placeItems: "center" }}><Trash2 size={12} /></button>
                      )}
                      <button title="Edit" style={{ width: 30, height: 30, border: "1px solid var(--line)", background: "transparent", color: "var(--muted)", cursor: "pointer", borderRadius: "var(--radius)", display: "grid", placeItems: "center" }}><Edit2 size={12} /></button>
                    </div>
                  </td>
                </motion.tr>
              );
