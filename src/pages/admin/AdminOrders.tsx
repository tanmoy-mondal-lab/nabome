...
import { generateMockAdminOrders } from "../../lib/mockAdminData";
...
export default function AdminOrders() {
  ...
  useEffect(() => { setOrders(generateMockAdminOrders()); }, []);

  const handleOverride = (id: string, newStatus: AdminOrder["status"]) => {
    setActionLoading(id);
    setTimeout(() => {
  ...
  const allStatuses: AdminOrder["status"][] = ["pending", "confirmed", "packed", "shipped", "delivered", "cancelled"];
  ...
  if (selected) {
    const config = statusConfig[selected.status] || { label: selected.status, color: "var(--muted)", bg: "transparent" };
    return (
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        ...
        <div className="glass" style={{ padding: 28, borderRadius: "var(--radius-xl)", marginBottom: 24 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", flexWrap: "wrap", gap: 16 }}>
            <div>
              ...
              <h2 style={{ fontSize: "1.3rem", fontWeight: 600 }}>{selected.orderId}</h2>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
          <div className="glass" style={{ padding: 24, borderRadius: "var(--radius-lg)" }}>
            ...
            <p style={{ color: "var(--muted)", fontSize: ".82rem", marginTop: 8 }}>Vendor: {selected.vendorName}</p>
          </div>
          <div className="glass" style={{ padding: 24, borderRadius: "var(--radius-lg)" }}>
            <h3 style={{ fontWeight: 600, fontSize: ".9rem", marginBottom: 12 }}>Shipping</h3>
            <p style={{ color: "var(--muted)", fontSize: ".82rem" }}>{selected.shippingAddress}</p>
            <p style={{ color: "var(--muted)", fontSize: ".82rem", marginTop: 8 }}>Payment: {selected.paymentMethod}</p>
          </div>
        </div>
        <div className="glass" style={{ padding: 24, borderRadius: "var(--radius-lg)", marginBottom: 20 }}>
          ...
          {selected.items.map((item, i) => (
            <div key={i} style={{ display: "flex", justifyContent: "space-between", padding: "8px 0", borderBottom: i < selected.items.length - 1 ? "1px solid var(--line)" : "none" }}>
              <span style={{ fontSize: ".85rem" }}>{item.name} × {item.quantity}</span>
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", paddingTop: 12, borderTop: "1px solid var(--line)", marginTop: 8 }}>
            ...
            <span style={{ fontWeight: 700, fontSize: "1.1rem", color: "var(--gold)" }}>₹{selected.total.toLocaleString("en-IN")}</span>
          </div>
        </div>
        {selected.note && (
          <div className="glass" style={{ padding: 24, borderRadius: "var(--radius-lg)", marginBottom: 20 }}>
            <h3 style={{ fontWeight: 600, fontSize: ".9rem", marginBottom: 8 }}>Note</h3>
            <p style={{ color: "var(--muted)", fontSize: ".85rem" }}>{selected.note}</p>
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {allStatuses.map((s) => {
            ...
            return (
              <button key={s} onClick={() => handleOverride(selected.id, s as AdminOrder["status"])} disabled={actionLoading === selected.id || s === selected.status}
                style={{
                  padding: "8px 16px", border: `1px solid ${s === selected.status ? cfg.color : "var(--line)"}`,
                  background: s === selected.status ? cfg.bg : "transparent",
                  color: s === selected.status ? cfg.color : "var(--muted)", cursor: s === selected.status ? "default" : "pointer",
                  borderRadius: "var(--radius)", fontSize: ".78rem", fontWeight: s === selected.status ? 700 : 500,
                  display: "flex", alignItems: "center", gap: 6, opacity: s === selected.status ? 0.6 : 1,
                }}
              >
                {actionLoading === selected.id && s !== selected.status ? <Loader2 size={12} className="spin" /> : null}
                {cfg.label}
              </button>
            );
          })}
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
        {filters.map((f) => <button key={f} onClick={() => setFilter(f)} style={chipS(filter === f)}>{f.charAt(0).toUpperCase() + f.slice(1)}</button>)}
      </div>
      ...
      <div style={{ overflowX: "auto" }}>
        <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 700 }}>
          ...
          <tbody>
            {filtered.map((o, i) => {
              const cfg = statusConfig[o.status] || { label: o.status, color: "var(--muted)", bg: "transparent" };
              return (
                ...
                >
                  <td style={{ padding: "12px 14px" }}>
                    <p style={{ fontWeight: 600, fontSize: ".85rem" }}>{o.orderId}</p>
                  ...
                  <td style={{ padding: "12px 14px", fontSize: ".82rem", color: "var(--muted)" }}>{o.vendorName}</td>
                  <td style={{ padding: "12px 14px", textAlign: "right", fontSize: ".85rem", fontWeight: 600, color: "var(--gold)" }}>₹{o.total.toLocaleString("en-IN")}</td>
