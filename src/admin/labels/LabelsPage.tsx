...
export default function LabelsPage() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {activeTab === "labels" && (labels.length === 0 && !loading ? <EmptyState title="No labels" description="Create product labels like 'New', 'Sale', 'Best Seller'." action={<button onClick={openCreate} className="bg-neutral-900 text-white px-4 py-2 text-sm rounded-lg transition-colors">Add Label</button>} />
        : <DataTable columns={[
      ...
      {activeTab === "tags" && (tags.length === 0 && !loading ? <EmptyState title="No tags" description="Create search tags for products." action={<button onClick={openCreate} className="bg-neutral-900 text-white px-4 py-2 text-sm rounded-lg transition-colors">Add Tag</button>} />
        : <DataTable columns={[
          { key: "name", label: "Name", sortable: true },
          { key: "slug", label: "Slug" },
          { key: "_count", label: "Products", render: (item) => <span className="text-xs bg-neutral-100 px-2 py-1 rounded">{(((item as Record<string, unknown>)._count as Record<string, number>)?.products ?? 0)}</span> },
