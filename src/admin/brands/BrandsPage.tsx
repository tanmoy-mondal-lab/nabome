...
export default function BrandsPage() {
  ...
  const [form, setForm] = useState({ name: "", description: "", logoUrl: "", websiteUrl: "", sortOrder: 0 });
  ...
  function openCreate() { setEdit(null); setForm({ name: "", description: "", logoUrl: "", websiteUrl: "", sortOrder: 0 }); setShowModal(true); }

  function openEdit(brand: Record<string, unknown>) {
    setEdit(brand as Record<string, string>);
    setForm({ name: brand.name as string ?? "", description: brand.description as string ?? "", logoUrl: brand.logoUrl as string ?? "", websiteUrl: brand.websiteUrl as string ?? "", sortOrder: (brand.sortOrder as number) ?? 0 });
    setShowModal(true);
  }
    ...
    <div className="p-6 space-y-6">
      ...
      {filtered.length === 0 && !loading ? <EmptyState title="No brands found" description="Add your first brand to get started." action={<button onClick={openCreate} className="bg-neutral-900 text-white px-4 py-2 text-sm rounded">Add Brand</button>} />
        : <DataTable columns={[
          ...
          { key: "logoUrl", label: "Logo", render: (item) => { const v = (item as Record<string, unknown>).logoUrl as string; return v ? <img src={v} alt="" className="w-10 h-10 object-contain rounded border" /> : null; } },
      ...
      <Modal open={showModal} title={edit ? "Edit Brand" : "Create Brand"} onClose={() => setShowModal(false)}>
        <div className="space-y-4">
          ...
          <div><label className="block text-xs text-neutral-500 mb-1">Description</label><textarea value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} rows={3} className="w-full px-3 py-2 text-sm border rounded focus:outline-none focus:ring-1 focus:ring-neutral-900" /></div>
          <div><label className="block text-xs text-neutral-500 mb-1">Logo URL</label><input value={form.logoUrl} onChange={(e) => setForm({ ...form, logoUrl: e.target.value })} className="w-full px-3 py-2 text-sm border rounded focus:outline-none focus:ring-1 focus:ring-neutral-900" /></div>
