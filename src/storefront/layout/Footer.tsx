    ...
    <footer className="bg-luxe-charcoal text-neutral-300">
      ...
      {footerConfig?.showNewsletter !== false && <div className="md:border-white/10 border-b border-white/5">
        <div className="container-wide md:py-12 py-8 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <p className="md:text-[10px] md:text-white/50 text-xs tracking-[0.2em] uppercase text-accent-goldLight mb-1">{(settings?.preferences?.newsletterTitle as string) || "Stay Connected"}</p>
            <p className="md:text-xl font-display text-lg text-white">{(settings?.preferences?.newsletterSubtitle as string) || "Join the নবME Inner Circle"}</p>
          </div>
      ...
      <div className="container-wide md:py-20 py-16">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 md:gap-8 lg:gap-12 gap-8">
          {/* Brand Column */}
              ...
              {socialLinks.map((link) => {
                ...
                return (
                  <a key={platform} href={link.url as string} target="_blank" rel="noopener noreferrer"
                    className="md:hover:border-white/30 md:hover:text-white md:hover:scale-110 w-10 h-10 border border-white/10 rounded-full flex items-center justify-center hover:border-accent-gold hover:text-accent-gold hover:scale-110 hover:shadow-lg hover:shadow-accent-gold/20 transition-all duration-300"
            ...
            <div key={i} className="col-span-1">
              {col.map((section) => {
                ...
                const content = typeof section.content === "string"
                  ? (() => {
                      try { return JSON.parse(section.content) as Record<string, unknown>; } catch { return {} as Record<string, unknown>; }
                    })()
