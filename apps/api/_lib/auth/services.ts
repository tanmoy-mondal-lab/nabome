/**
 * Authentication services — JWT + bcrypt implementation (V1 auth).
...
import {
  ...
  buildEmailConfig as getEmailConfig,
...
function getJwtSecret(): string {
  ...
  if (!s) throw ApiError.internal('Server misconfigured: JWT_SECRET missing');
  return s;
}
...
export async function register(input: RegisterInput): Promise<RegisterResult> {
  const { email, password, firstName, lastName, locale = 'en-IN' } = input;
  ...
  if (existingUser) {
    ...
    // If email exists, we still return success but don't create duplicate
    // The user will get an email verification link if they need to verify
    return {
      user: existingUser,
      requiresEmailVerification: !existingUser.emailVerifiedAt,
    };
  }
  ...
  try {
    const emailConfig = getEmailConfig({} as any);
...
export async function refreshSession(
  ...
  try {
    payload = verifyToken(refreshToken, getJwtSecret());
  } catch {
...
export async function confirmPasswordReset(
  ...
  // Delete password reset record (single-use token)
