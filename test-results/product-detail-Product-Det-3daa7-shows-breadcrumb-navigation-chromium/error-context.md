...
- Name: product-detail.spec.ts >> Product Detail Page >> product detail shows breadcrumb navigation
- Location: e2e/product-detail.spec.ts:100:3
...
```ts
  2   | 
  3   | test.describe('Product Detail Page', () => {
  4   |   test('product detail page loads for valid product', async ({ page }) => {
  ...
  10  |   });
  11  | 
  12  |   test('product detail shows product name', async ({ page }) => {
