  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (request.method === 'POST' && url.pathname === '/run') {
      ...
      if (
        ...
        secret !== env.SETTLEMENT_CRON_SECRET
      ) {
        return Response.json({ error: 'Unauthorized' }, { status: 401 });
      }
      const result = await handleScheduled(env);
      const isError =
        result.errors > 0 && result.created === 0 && result.processed === 0;
...
async function handleScheduled(env: Env): Promise<SettlementRunResult> {
  ...
  const res = await fetch(`${apiUrl}/api/v1/internal/settlement/run`, {
    method: 'POST',
    headers: {
      'x-settlement-secret': secret,
      'content-type': 'application/json',
    },
    body: JSON.stringify({ actorId: 'system-cron' }),
    signal: AbortSignal.timeout(30000),
  });
  const body = (await res.json().catch(() => ({}))) as SettlementApiResponse;
  if (!res.ok) {
    console.error('[settlement] API error', res.status, body);
    throw new Error(
      `settlement API failed: ${res.status} ${JSON.stringify(body).slice(0, 500)}`,
    );
  }
  console.log(
    `[settlement] API result ${JSON.stringify(body)} ${Date.now() - start}ms`,
  );
  return {
    processed: body.data?.processed ?? 0,
    created: body.data?.created ?? 0,
    errors: body.data?.errors ?? 0,
